defmodule SymphonyElixir.CLI do
  ...
  @type deps :: %{
          ...
          set_instance_config_file_path: (String.t() -> :ok | {:error, term()}),
  ...
  def run(instance_config_path, deps) do
    expanded_path = Path.expand(instance_config_path)

    if deps.file_regular?.(expanded_path) do
      :ok = deps.set_instance_config_file_path.(expanded_path)
      ...
      case deps.ensure_all_started.() do
        ...
        {:error, reason} ->
          {:error, "Failed to start Symphony with instance_config #{expanded_path}: #{inspect(reason)}"}
      end
    else
      {:error, "instance_config file not found: #{expanded_path}"}
    end
  end
