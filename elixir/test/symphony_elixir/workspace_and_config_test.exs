defmodule SymphonyElixir.WorkspaceAndConfigTest do
  ...
  test "workspace path is deterministic per issue identifier" do
    ...
    write_instance_config_file!(InstanceConfig.instance_config_file_path(), workspace_root: workspace_root)
  ...
  test "relative local workspace roots resolve from the instance_config directory" do
    instance_config_dir = Path.dirname(InstanceConfig.instance_config_file_path())
    ...
    try do
      ...
      write_instance_config_file!(InstanceConfig.instance_config_file_path(), workspace_root: "relative-workspaces")
  ...
  test "workspace canonicalizes symlinked workspace roots before creating issue directories" do
    ...
    try do
      ...
      write_instance_config_file!(InstanceConfig.instance_config_file_path(), workspace_root: linked_root)
  ...
  test "workspace cleanup handles missing workspace root" do
    ...
    write_instance_config_file!(InstanceConfig.instance_config_file_path(), workspace_root: missing_root)
  ...
  test "linear graphql honors a bound tracker-settings snapshot without loading live config" do
    ...
    instance_config_store_pid = Process.whereis(InstanceConfigStore)
    ...
    on_exit(fn ->
      ...
      if is_pid(instance_config_store_pid) and is_nil(Process.whereis(InstanceConfigStore)) do
        Supervisor.restart_child(SymphonyElixir.Supervisor, InstanceConfigStore)
      end
    end)

    if is_pid(Process.whereis(InstanceConfigStore)) do
      assert :ok = Supervisor.terminate_child(SymphonyElixir.Supervisor, InstanceConfigStore)
    end

    InstanceConfig.set_instance_config_file_path(missing_instance_config_path)
  ...
  test "issue assigned to another worker is not dispatch-eligible" do
    write_instance_config_file!(InstanceConfig.instance_config_file_path(), tracker_assignee: "dev@example.com")
  ...
  test "config reads defaults for optional settings" do
    ...
    write_instance_config_file!(InstanceConfig.instance_config_file_path(), tracker_required_labels: [" "])
    ...
    write_instance_config_file!(InstanceConfig.instance_config_file_path(), tracker_active_states: ",")
    assert {:error, {:invalid_instance_config_config, message}} = Config.validate!()
    assert message =~ "tracker.active_states"

    write_instance_config_file!(InstanceConfig.instance_config_file_path(), max_concurrent_agents: "bad")
    assert {:error, {:invalid_instance_config_config, message}} = Config.validate!()
    assert message =~ "agent.max_concurrent_agents"

    write_instance_config_file!(InstanceConfig.instance_config_file_path(), worker_max_concurrent_agents_per_host: 0)
    assert {:error, {:invalid_instance_config_config, message}} = Config.validate!()
    assert message =~ "worker.max_concurrent_agents_per_host"

    write_instance_config_file!(InstanceConfig.instance_config_file_path(), codex_turn_timeout_ms: "bad")
    assert {:error, {:invalid_instance_config_config, message}} = Config.validate!()
    assert message =~ "codex.turn_timeout_ms"

    write_instance_config_file!(InstanceConfig.instance_config_file_path(), codex_read_timeout_ms: "bad")
    assert {:error, {:invalid_instance_config_config, message}} = Config.validate!()
    assert message =~ "codex.read_timeout_ms"

    write_instance_config_file!(InstanceConfig.instance_config_file_path(), codex_stall_timeout_ms: "bad")
    assert {:error, {:invalid_instance_config_config, message}} = Config.validate!()
    ...
    write_instance_config_file!(InstanceConfig.instance_config_file_path(), codex_approval_policy: "")
    assert :ok = Config.validate!()
    assert Config.settings!().codex.approval_policy == ""

    write_instance_config_file!(InstanceConfig.instance_config_file_path(), codex_thread_sandbox: "")
    assert :ok = Config.validate!()
    assert Config.settings!().codex.thread_sandbox == ""

    write_instance_config_file!(InstanceConfig.instance_config_file_path(), codex_turn_sandbox_policy: "bad")
    assert {:error, {:invalid_instance_config_config, message}} = Config.validate!()
    ...
    write_instance_config_file!(InstanceConfig.instance_config_file_path(), codex_command: "codex app-server")
  ...
  test "config supports per-state max concurrent agent overrides" do
    ...
    write_instance_config_file!(InstanceConfig.instance_config_file_path(), worker_max_concurrent_agents_per_host: 2)
    assert :ok = Config.validate!()
  ...
  test "instance_config contains runtime configuration but no prompt body" do
